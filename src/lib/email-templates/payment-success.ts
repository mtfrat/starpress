export function paymentSuccessTemplate(plan: string): string {
  return `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 20px rgba(12,23,84,0.06);">
          <tr>
            <td style="padding:40px;text-align:center;">
              ...
              <h1 style="margin:0 0 8px;font-size:28px;font-weight:600;color:#0c1754;">You're now on ${plan}!</h1>
              <p style="margin:0 0 32px;font-size:14px;color:#666;line-height:1.6;">
                Your payment was successful. You now have access to all ${plan} features including unlimited locations, widgets, and AI analysis.
              </p>
              <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://starpress.app'}/dashboard" style="display:inline-block;padding:14px 32px;background-color:#2545ff;color:#ffffff;text-decoration:none;border-radius:50px;font-size:14px;font-weight:600;">Go to Dashboard →</a>
              <p style="margin:32px 0 0;font-size:12px;color:#999;line-height:1.5;">
                Manage your billing anytime from Settings → Billing.
              </p>
            </td>
          </tr>
        </table>
